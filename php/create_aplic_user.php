<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($fio && $phone && $visit && $kol_people && $date && $time) {
            ...
            $applications_create = mysqli_query($connect, "INSERT INTO `applications` 
            (`user_id`, `fio`, `telephone`, `visiting_id`, 
            `quantity`, `data`, `time`, `status_id`) VALUES 
            ('$id_user', '$fio', '$phone', '$visit', '$kol_people', '$date', '$time', 3)");
    
            // Проверка успешности выполнения запроса
            if ($applications_create) {
                header('Location: ../my_applications/my_applications_all.php');
                exit();
            } else {
